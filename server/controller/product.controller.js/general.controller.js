  ...
  try {
    ...
    if (user.wallet_balance < invest_amount) {
      return response_message(
        ...
        400,
        false,
        "Insufficient balance in recharge wallet. Please recharge.",
        null
      );
    }
    ...
    const user_product = await user_product_model.findOne({
      user_id: user_id,
      product_id: product_id,
    });
    ...
    if (referral_data.activated_bonus === false) {
      const referral_user_o = await register_model.findOne({
        _id: referral_data.user_referral_o.user_id_o,
      });
      console.log("referral_user_o register user", referral_user_o);
      const amount = invest_amount * 0.1;
      referral_user_o.referal_income += amount;
      referral_user_o.withdrawl_balance += amount;

      await referral_user_o.save();

      referral_data.user_referral_o.referral_amount = invest_amount * 0.1;
